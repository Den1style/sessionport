...
### Запланировано
- Кросс-платформенная синхронизация Chrome ↔ iOS (Pro) — через ту же Drive-папку.
- Расширенная аутентификация Google (профиль, статус подписки).

---

## [v1.0.5] — 2026-06-17

### Реальная синхронизация через Google Drive (между браузерами)

- **Двусторонний синк по каноническому файлу** `sessionport-sync.json` в *вашем* Google Drive: при открытии расширения снимки подтягиваются с Drive и отправляются обратно (pull → merge → push). Несколько браузеров на одном Google-аккаунте сходятся к одной базе.
- **Слияние без потерь:** снимки объединяются по `snapshot_id`; удаления/восстановления распространяются между устройствами по правилу last-write-wins через новое поле `state_at`.
- **Автосинк при открытии** попапа (не чаще раза в минуту), плюс начальный синк сразу после подключения Google Drive.
- **Приватность:** данные идут напрямую браузер ↔ ваш Drive (scope `drive.file` — только файлы расширения). У SessionPort по-прежнему нет серверов и копий ваших данных. Privacy policy обновлён.
- Периодические резервные копии (Settings → Backup) работают как прежде — это дополняющий механизм точечного восстановления.
